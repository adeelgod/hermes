...
java -Dspring.profiles.active="debug" -Dlogback.configurationFile=$HERMES_CONFIG_BASE/logback.xml -Dhermes.config="$HERMES_CONFIG" -Dcommon.config="$COMMON_CONFIG" $JPDA_OPTS -Xmx512m -Xms512m -jar target/hermes-rest-server-ui-1.0.3-SNAPSHOT-shaded.jar --port "8081" --jersey "/api/*" --cors "/api/*"

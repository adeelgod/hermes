...
java -Dspring.profiles.active="debug" -Dlogback.configurationFile=$HERMES_CONFIG_BASE/logback.xml -Dhermes.config="$HERMES_CONFIG" $JPDA_OPTS -Xmx512m -Xms512m -jar target/hermes-rest-server-1.0.3-SNAPSHOT-shaded.jar --port "8081" --jersey "/api/*" --cors "/api/*"
